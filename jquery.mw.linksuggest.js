/*
 * jQuery MediaWiki LinkSuggest 1.0.0
...
$.widget( "mw.linksuggest", {
	...
	_create: function() {
		...
		.bind( 'keydown.linksuggest', function( thisInstance ) {
			return function() {
	...
	_sendQuery: function( request, response ) {
		var emptyset = [];
		var text = this.element.val().replace( /\r/g, '' );
		var caret = this._getCaret( this.element[0] );
		var sQueryStartAt = -1;
		var sQueryReal = '';
		var format = '';
	...
	_updateValue: function( oItem ) {
		...
		this._setCaret( this.element[0], textBefore.length + oItem.value.length );
		this.element[0].scrollTop = scrollTop;
	},
	_setCaret: function( control, pos ) {
	...
	_getCaretPosition: function( control ) {
		...
		for (var i = 0; i < props.length; i++) {
			tester.css( props[i], $ctrl.css( props[i] ) );
		}
		...
		tester.width( control.clientWidth ).appendTo( document.body ).text( $ctrl.val().substr( 0, caret ) );
		...
		tester.text( $ctrl.val().substr( 0, initialcaret ) + ' ' );
	...
	_open: function( event, ui ) {
		...
		var offset = this._getCaretPosition( this.element[0] );
